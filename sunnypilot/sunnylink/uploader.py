...
class Uploader:
  ...
  def upload(self, name: str, key: str, fn: str, network_type: int, metered: bool) -> bool:
    ...
    kmsg_log("sunny_uploader", "upload_start", mono_ns=mono_ns, key=key, fn=fn,
      ...
      if stat is not None and stat.status_code in (200, 201, 412):
        ...
        if stat.status_code == 412:
          ...
          cloudlog.event("upload_ignored", key=key, fn=fn, sz=sz, network_type=network_type,
                         metered=metered, mono_ns=mono_ns)
          kmsg_log("sunny_uploader", "upload_ignored", mono_ns=mono_ns, key=key, fn=fn,
                   sz=sz, network_type=network_type, metered=metered)
        else:
          ...
          cloudlog.event("upload_success", key=key, fn=fn, sz=sz, content_length=content_length,
                         ...
                         mono_ns=mono_ns)
          kmsg_log("sunny_uploader", "upload_success", mono_ns=mono_ns, key=key, fn=fn,
        ...
        cloudlog.event("upload_failed with content", stat=stat, exc=last_exc, key=key, fn=fn, sz=sz, network_type=network_type, metered=metered,
                       error=stat.content.decode("utf-8"), mono_ns=mono_ns)
        kmsg_log("sunny_uploader", "upload_failed", mono_ns=mono_ns, key=key, fn=fn,
                 sz=sz, network_type=network_type, metered=metered,
                 status_code=stat.status_code, level=4)
      else:
        success = False
        mono_ns = mono_time_ns()
